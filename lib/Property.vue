<template lang="html">
    <div v-if="fullSchema" class="vjsf-property">
        ...
        <!-- Date picker -->
        <v-menu v-else-if="fullSchema.type === 'string' && ['date', 'date-time'].includes(fullSchema.format)" ref="menu"
        ...
        >
            ...
            <v-date-picker v-model="modelWrapper[modelKey]" no-title scrollable :locale="options.locale">
                ...
                <v-btn flat color="primary" @click="$refs.menu.save(modelWrapper[modelKey])">
        ...
        <!-- radio group based on an enum (array or enum and x-display=radio) -->
        <template
     ...
     <!-- signpad simple string and x-signpad=true -->
        ...
        <v-select
                v-else-if="(fullSchema.type === 'array' && ['string', 'integer', 'number'].includes(fullSchema.items.type) && fullSchema.items.oneOf) || (['string', 'integer', 'number'].includes(fullSchema.type) && fullSchema.oneOf)"
        ...
        >
            ...
            <template slot="no-data">
                <v-list-tile>
                    <v-list-tile-content>
                        <v-list-tile-title>
                          Escriba y presione <kbd>enter</kbd> para crear una nueva opción.
                        </v-list-tile-title>
                    </v-list-tile-content>
                </v-list-tile>
            </template>
                ...
                <div v-show="!foldable || !folded">
                    ...
                    <template v-if="fullSchema.allOf && fullSchema.allOf.length">
                        <template v-if="!parentKey && fullSchema.allOf[0].title">
                            ...
                            <v-expansion-panel :inset="options.accordionMode === 'inset'"
                                               :popout="options.accordionMode === 'popout'" focusable>
        ...
        <div v-else-if="fullSchema.type === 'array'">
            <v-layout row class="mt-2 mb-1 pr-1">
                ...
                <v-btn v-if="!disabled" icon color="primary"
                       @click="modelWrapper[modelKey].push(fullSchema.items.default || defaultValue(fullSchema.items))">
                    <v-icon>add</v-icon>
                </v-btn>
                <v-spacer/>
                <v-tooltip v-if="fullSchema.description" left>
